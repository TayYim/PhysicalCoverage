...
def random_select(number_of_tests):
    ...
    ALL_POSSIBILITIES = int(3 * 2 ** ((60/4)**2)) # TODO: This is a magic number
    ...
    coverage_percentage = (float(len(seen_fingerprint_set)) / ALL_POSSIBILITIES) * 100
...
fingerprints_file_names = glob.glob(base_path + "fingerprints_*")
...
fingerprint_file = fingerprints_file_names[0]
...
test_suit_sizes = [10, 50, 100]

f = open("Fingerprint wirh RSS{}-{}.txt".format(RRS_number, args.scenario), "w")
...
plt.xlabel("Fingerprint Coverage (%)")
