...
from process_functions import processFingerprint
...
# TODO: 数据格式可能会溢出,U100为100位
# TODO: Change this to 3, adding TTR
dt = np.dtype([('integer', np.int8), ('string', 'U100')])
fingerprints       = np.empty((total_files, vec_per_file), dtype=dt) 
...
for i in range(total_files):
    ...
    file_name = file_names[i]
    # Start the job
    jobs.append(pool.apply_async(processFingerprint, args=([file_name, vec_per_file])))

# Get the results
for i, job in enumerate(tqdm(jobs)):
    result = job.get()
    this_fingerprints, edge_length, cell_size, file_name = result

    fingerprints[i] = this_fingerprints

# Close your pools
pool.close()

save_name = args.scenario
save_name += "_l" + str(edge_length) 
save_name += "_c" + str(cell_size) 
...
print("Saving data")
np.save(save_path + '/fingerprints_{}'.format(save_name), fingerprints)